...
func ValidatePasswordPlainText(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 8 && len(password) <= 72, "password", "must be between 8 and 72 characters")
}

func ValidateUser(v *validator.Validator, user *User) {
	v.Check(user.Name != "", "user", "must be provided")
	v.Check(len(user.Name) <= 500, "user", "must not be more than 500 bytes")
...
func (userModel UserModel) Update(user *User) error {
	...
	err := userModel.DB.QueryRowContext(ctx, query, args...).Scan(&user.Version)
